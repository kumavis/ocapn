...
export const makeSimNetlayerFactory = ({
  ...
  return (handlers, logger) => {
     ...
     *
     * @param {string} key  - identity for the outgoing map (peer designator)
     * @param {() => MessagePort | undefined} getPort
     * @returns {import('@endo/ocapn').SocketOperations}
     */
    const makeSocketOps = (key, getPort) => {
      let closed = false;
      return {
        write(bytes) {
          if (closed) return;
          ...
          const dispatch = () => {
            ...
            if (livePort) {
              send(livePort);
            } else if (port === undefined && key !== undefined) {
              // Buffer until the port arrives.
              const slot = outgoing.get(key);
          ...
          const port = getPort();
          if (port) {
            try { port.postMessage({ kind: 'close' }); } catch {}
    ...
    const wireIncomingPort = (peerDesignator, port) => {
      let connection;
      const socketOps = makeSocketOps(undefined, () => port);
      ...
      port.onmessage = ev => {
        ...
        if (data?.kind === 'data') {
          ...
          if (latency > 0) sleep(latency).then(deliver); else deliver();
    ...
    const connect = target => {
      if (target.transport !== 'simworker') {
        throw Error(`sim-netlayer cannot connect to transport: ${target.transport}`);
      }
      ...
      const socketOps = makeSocketOps(key, () => slot.port);
    ...
    const shutdown = () => {
      for (const slot of outgoing.values()) {
        if (slot.port) {
          try { slot.port.postMessage({ kind: 'close' }); } catch {}
          try { slot.port.close(); } catch {}
        }
        try { slot.connection.end(); } catch {}
      }
      outgoing.clear();
    };
